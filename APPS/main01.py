...
#host = '213.32.89.50'
host = '192.168.1.200'
port = 6616                                                                                                             # tu zmienic !!!
port2 = 6716                                                                                                            # tu zmienic !!!
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            BoxLayout:
                ...
                size_hint: 1, .18
                Label:
                    text: '[b]PRESSING ROOM - 01[b]'                                                                    # tu zmienic !!!
                    markup: 'True'
...
<InfoWindow>:
    ...
    size_hint: 0.8, 0.8
    title_align: 'center'
    title: 'BOARDS TO PRESS'                                                                                            # tu zmienic
    BoxLayout:
        orientation: 'horizontal'
        BoxLayout:
            orientation: 'vertical'
...
class InfoWindow(Popup):
    ...
    def popup_count(self):
        ...
        while co < 20:
            ...
            if (co > 9):
                mo = str(co)
            code_win[co].text = str(collection_topress.find({"model": mo}).count())                                     #tu zmienic !!
            co += 1
...
#        threading.Thread(target=self.message_read).start()
    ...
    def data_check(self):                                                                                               # tu_zmienic !!!
        ...
        check_counter = 0
        while (is_present == 0):
            is_present = collection_01.find({"code": read_code}).count()
            if (is_present > 0):
                data_added = 1
                break
            else:
    ...
    def comm_check(self):                                                                                               # tu zmienic !!!
        ...
        check_counter = 0
        while (is_present == 0):
            is_present = collection_tocut.find({"code": read_code}).count()
            if (is_present > 0):
                comm_added = 1
                break
            else:
        ...
        if (data != 0 and data != -1):
            ...
            if (len(worker_name) > 2 and sec == 0 and com == 0):
                ...
                if (worker != ""):
                    if (sec == 0 and com == 0):
                        if (data[3:5] == "99"):
                            press_number = data[5:7]
                            mold_model = data[13:15]
                            name = "mold"+press_number
                            collection_molds.delete_many({name: {'$exists': True}})
                            mold_data = {
                                name: mold_model,
                            }
                            collection_molds.insert_one(mold_data)
                            status_label.text = 'MOLD CHANGED'
                            current_code.text = mold_model
                            current_time.text = timestamp[11:19]
                        else:
                            if (collection_01.find({"code": read_code}).count() == 0):                                      # tu zmienic !!!
                                new_data = {
                                    ...
                                    "model": model,
                                    "worker": worker,
                                    "operation": "01",                                                                      # tu zmienic !!!

                                }
                                collection_01.insert_one(new_data)                                                          # tu zmienic !!!
                                print("added to 01 base : " + read_code)                                                    # tu zmienic !!!
                                ...
                                collection_tocut.insert_one(new_data2)                                                      # tu zmienic !!!
                                i = db.topress.delete_many({"code": read_code}).deleted_count                               # tu zmienic !!!  uwaga przy 00
                                ...
                                if (i < 1):
                                    missing_code = {
                                        ...
                                        "code": read_code,
                                        "model": model,
                                        "missing": "00",                                                                    # tu zmienic !!!
                                        "timestamp": timestamp
                                    }

                                    collection_missingcodes.insert_one(missing_code)
                                    print("missing in 00 base : " + read_code)                                              # tu zmienic !!!
